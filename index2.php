...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_flag"])){ 
  $type_update = filter_var($_POST["car_type"], FILTER_SANITIZE_STRING);
        ...
        if($result->num_rows > 0){
          while($row = $result->fetch_assoc()){ // start loop
            ...
            if(isset($_GET["update_id"]) && $_GET["update_id"] == $row['id']){
              ...
              echo "<input type='hidden' name='update_flag' value=".$row['id'] .">";
              echo "<td><input name='car_type' value=".$row["type"]."> </td>";
              echo "<td><input name='car_engine' value=".$row["engine"]."> </td>";
              echo "<td><input name='car_year' value=".$row["year"]."> </td>";
              echo "<td><input name='car_fuel' value=".$row["fuel"]."> </td>";
              echo "<td><input name='car_model' value=".$row["model"]."> </td>";
              echo "<td><input name='car_make' value=".$row["make"]."> </td>";
            ...
            <tr>
                ...
                <td><a href="<?php echo  $_SERVER['PHP_SELF'];?>?update_id=<?php echo $row['id']?>"> update</a></td>
